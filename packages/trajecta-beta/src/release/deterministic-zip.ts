...
export const MAX_CANONICAL_ZIP_BYTES = MAX_BYTES + MAX_MEMBERS * 600;
...
/** Reads only the first untrusted DOS timestamp so canonical ZIP decoding can bind it to the signed receipt. */
export function readZipReleaseInstant(zip: Buffer): string {
  if (!Buffer.isBuffer(zip) || zip.length < 30 || zip.length > MAX_CANONICAL_ZIP_BYTES || zip.readUInt32LE(0) !== 0x04034b50) fail();
  ...
  if (month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59
    ...
    || instant.getUTCHours() !== hour || instant.getUTCMinutes() !== minute || instant.getUTCSeconds() !== second) fail();
  const rendered = instant.toISOString().replace('.000Z', 'Z');
  parseReleaseInstant(rendered);
  return rendered;
}
...
export function readZip(zip: Buffer, instant: unknown): ZipMember[] {
  if (!Buffer.isBuffer(zip) || zip.length < 22 || zip.length > MAX_CANONICAL_ZIP_BYTES) fail();
