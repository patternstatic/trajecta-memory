...
import { buildCommercialCandidateReceipt, buildEvaluationReceipt, signReceipt } from './signing.ts';
import { type TarLedgerMember } from './tar-reader.ts';
import { releaseError } from './errors.ts';
import fs from 'node:fs';
import path from 'node:path';
import { assertNewOutputDirectory, resolveReleaseKind, type ReleaseKind } from './contracts.ts';
...
export function assembleBundle(input: AssemblyInput): { zip:Buffer; archiveSha256:string; publicKeyFingerprint:string } {
  ...
  const receiptInput={buildCommit:input.buildCommit,manifestBytes:manifest.bytes,releaseInstant:input.releaseInstant,verificationInstant:input.verificationInstant,publicKeyPem:input.publicKeyPem};
  const receipt = resolveReleaseKind(input.releaseKind)==='commercial-candidate' ? buildCommercialCandidateReceipt(receiptInput) : buildEvaluationReceipt(receiptInput);
